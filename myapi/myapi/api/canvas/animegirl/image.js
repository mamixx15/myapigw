  ...
  async run(req, res) {
    try {
      ...
      if (!fs.existsSync(fontPath))
        return res
          .status(500)
          .json({ error: "Font not found (LEMONMILK-Bold.otf)" });
      ...
      const startY =
        boardY + (boardHeight - totalTextHeight) / 2 + fontSize / 2 + 60;
      ...
      // ✅ Simpan sementara di /tmp (karena read-only di Vercel)
      ...
      // ✅ Domain otomatis dari request
      const domain = `${req.protocol}://${req.get("host")}`;
      const fileUrl = `${domain}/files/${finalFileName}`;

      // Hapus otomatis 5 menit
      setTimeout(() => {
