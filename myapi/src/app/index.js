 ...
 * @name GET /admin/banned-list
 * @description Get all banned IPs. Requires valid admin key.
 * @route {GET} /admin/banned-list
 * @param {express.Request} req
 * @param {express.Response} res
 * @header {string} X-Admin-Key
 * @returns {Object}
 * @example
 * {
 *   "success": true,
 *   "count": 1,
 *   "banned": { "1.2.3.4": { "bannedAt": "...", "reason": "...", "by": "rateLimiter" } }
 * }
 */
app.get("/admin/banned-list", (req, res) => {
  const key = req.headers["x-admin-key"];
  if (key !== process.env.ADMIN_KEY)
    return res.status(401).json({ success: false, error: "Unauthorized" });

  const banned = rateLimiter.getBannedList();
  res.json({ success: true, count: Object.keys(banned).length, banned });
}); 
